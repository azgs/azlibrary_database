	...
	.then(() => {
		...
		return readdirPromise(dir).then(subElements => {
			...
			.then((metadataIDs) => {
				...
				logger.silly("layers metadataIDs = " + global.pp(metadataIDs));
				...
				logger.silly("layers metadataIDs stripped = " + global.pp(metadataIDs));

				//Process each layers file/dir
				const promises = subElements.map((element) => {
					logger.silly("processing layers element " + element);
					...
					return statPromise(path.join(dir,element)).then(stats => {
						...
						if (stats.isDirectory()) {
							if (suffix === "SHP") {
								logger.silly("processing layers shape directory " + element);
								...
								logger.warn("layers directory " + element + " is not a shape directory. Ignoring");
								return Promise.resolve();
							}
						...
						logger.silly("processing layers file " + file);
						...
						if (metadataID === null) {
							logger.warn("No metadata file found for layers file " + file);
						}
						...
						.then((extent) => {
							...
							//Insert layers record for this file, creating bbox from extent
							return db.none("insert into gisdata.layers (collection_id, metadata_id, name, path, geom, bbox_from_meta) values (" +
											collectionID + "," +
											metadataID + "," + 
											...
											extent.fromMeta + 
							")").catch(error => {logger.error("problem inserting layers record:");logger.error(error); return Promise.reject(error);});
						}).catch((error) => {
							logger.error("Problem creating layers record for collection " + collectionID + ", file " + file);
							logger.error(error);
							return Promise.reject(error);
						});
					}).catch(error => {logger.error("problem getting stats for element " + element);logger.error(error); return Promise.reject(error);});
				})
				return Promise.all(promises).catch(error => {logger.error("Problem processing layers files");logger.error(error); return Promise.reject(error);});
		...
		logger.error("problem handling layers directory. " + global.pp(error));
		return Promise.reject(error);
	});
};
...
function fetchExtent(collectionID, dir, file, db) {
	...
	return new Promise((resolve, reject) => {
		...
		try {
			...
			db.one("select min(srid) as srid from public.spatial_ref_sys where auth_name='EPSG' and trim(proj4text) = trim('" + extent.srid + "')")
			.then((data) => {
